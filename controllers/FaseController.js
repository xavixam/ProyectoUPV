...
const FaseController = {
    ...
    //         // Paso 2: Crear la nueva fase usando el _id del trabajador y el _id del lote encontrado
    ...
    //     } catch (error) {
    //         console.error(error);
    //         res.status(500).send({ message: "Hubo un problema al procesar la solicitud", error });
    //     }
    // },
    async create(req, res) {
        try {
            ...
            const fasesActualizadas = await Fase.find({ loteId: lote._id, terminado: true }); // Obtenemos las fases actualizadas
            const historiales = [];
            ...
            if (historiales.length > 0) {
                ...
                console.log("No se agregaron nuevos registros al historial (todos ya existen).");
            }
